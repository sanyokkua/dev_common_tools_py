...
def generate_git_config_commands(user_name: str, user_email: str, is_global: bool = True) -> str:
    if not user_name or not user_email or len(user_name.strip()) == 0 or len(user_email.strip()) == 0:
        raise UserNameOrEmailIsEmptyException(
            "Passed user name ({}) and email ({}) are not correct".format(user_name, user_email))
    global_flag: str = " --global " if is_global else " "
    git_config_name: str = GIT_CONFIG_USER_NAME.format(global_flag, user_name.strip())
    git_config_email: str = GIT_CONFIG_USER_EMAIL.format(global_flag, user_email.strip())
    ...
    return su.join_lines([commands_separate, "", commands_to_execute_batch], separator="\n")
